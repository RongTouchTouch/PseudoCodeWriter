...
def test(args,ds,m,epoch='cmdline'):
  ...
  os.system("cd /home/zhy/testresult")
  os.system("vi " + model + ".input.beam_predictions." + epoch)
  
  ofn = dir + "/testresult/"+model+".beam_predictions"
  m.eval()
  k = 0
  data = ds.mktestset(args)
  ofn = dir + "/testresult/"+model+".inputs.beam_predictions."+epoch
  pf = open(ofn,'w')
  preds = []
  golds = []
  for b in data:
