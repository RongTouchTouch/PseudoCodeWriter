...
class dataset:
  ...
  def mkVocabs(self,args):
    ...
    specials = "variable system interface hardware constant statevariable message time state mode beacon cycle otheritem".split(" ")
    for x in specials:
      for y in range(40):
        s = "<"+x+"_"+str(y)+">"
    ...
    self.INP.build_vocab(train, min_freq=0)
