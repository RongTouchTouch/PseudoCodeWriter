...
class dataset:
  ...
  def mkVocabs(self,args):
    ...
    self.fields=[("index",self.INDEX),("src",self.INP),("ent",self.ENT),("nerd",self.NERD),("rel",self.REL),("out",self.OUTP),("sorder",self.SORDER)]
    ...
    generics =['<method>','<material>','<otherscientificterm>','<metric>','<task>','<variable>','<system>','<interface>','<hardware>','<otheritem>']
    ...
    specials = "variable system interface hardware otheritem".split(" ")
    for x in specials:
      for y in range(40):
        s = "<"+x+"_"+str(y)+">"
